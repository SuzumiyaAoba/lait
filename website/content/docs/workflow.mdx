...
description: "nodes/steps の分離とノードの再利用、モデル定義、JSON 出力と jq による加工、write_file、MCP ツールの利用、スキルの利用、steps 参照、retry/timeout/on_error、when/switch、parallel、loop、for_each、stop/break、サブワークフロー呼び出し"
---
