...
- [ワークフロー（workflow.yml）](./docs/usage/ja/workflow.md) — step の逐次実行、ワークフロー内でのモデル定義、JSON 出力と `jq` による加工、条件分岐（`when`/`switch`）、並列実行（`parallel`）、条件ループ（`loop`）、配列反復（`for_each`）、早期終了（`stop`/`break`）
