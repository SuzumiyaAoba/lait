...
`--record` で作った `<DIR>` を指定すると、LLM API を呼ばずに、記録済みレスポンスをリクエスト内容に基づいて返します。ワークフロー内の MCP サーバー、`run` ステップ、shell tool、ファイル操作などは通常どおり実行されるため、それらも含めて外部 I/O を止めたい場合はテスト用の fixture/mock を用意してください。

```sh
$ lait run workflow.yml "プロンプト" --replay ./cassettes
```

- リクエストの内容(ベース URL・モデル・メッセージ履歴・ツール定義・response format)が記録時と完全に一致した場合のみカセットがヒットします。ワークフローや `--input` を変更してリクエスト内容が変わると、以前のカセットはヒットしなくなります。
...
テスト定義 YAML を一括実行し、pass/fail を報告します。ファイル/ディレクトリの両方を指定でき、ディレクトリは再帰的に `.yml`/`.yaml` を探索します(隠しファイル・隠しディレクトリ、および `target`/`node_modules` ディレクトリはスキップ)。探索結果はパス順に並び、同じ実体を複数の引数(ディレクトリとファイルの重複など)から指定しても1回だけ実行されます。
...
複数のテストファイルは最大8件まで同時に実行されます。各ファイルはそれぞれ独立したワークフロー実行なので、あるファイルの `workflow:` に `write:` ステップがあり、かつ別のテストファイルの `workflow:` も**同じ相対パス**に書き込む場合、両者が同時に走ると競合します(`for_each` の `max_concurrency` が2以上の本体で固定パスへの `write` を禁止しているのと同種の問題ですが、こちらは静的検査の対象外です)。テスト定義ごとに書き込み先を分けるか、`ScratchDir` 相当の使い捨てディレクトリを使ってください。
...
input: "要約したい長文..."     # 省略可(既定は null)。ワークフローの最初の入力
inputs:                       # 省略可。ワークフローの inputs: に渡される({{ inputs.<name> }})
  lang: ja
...
| フィールド | 説明 |
| --- | --- |
| `workflow` | テスト対象のワークフローファイル（このテスト定義ファイルからの相対パス）。 |
| `input` | 省略可（既定は `null`）。ワークフローへ渡す初期値。YAML の値がそのまま（型付きで）渡され、ワークフローの `input_schema:` で検証されます。 |
| `inputs` | 省略可。ワークフローが宣言した `inputs:` に渡す値のマップ。YAML の値がそのまま（型付きで）渡され、各値は `inputs:` の宣言で検証されます。 |
| `replay` | 必須。`lait run --record` で事前に作ったカセットディレクトリへの相対パス（このテスト定義ファイルからの相対パス）。 |
| `assert` | 出力・実行軌跡に対するアサーションのリスト。 |

`assert:` の各項目の `type:` に指定できる値は次のとおりです。

| `type` | 主なフィールド | 説明 |
| --- | --- | --- |
| `equals` | `value` | 出力が `value` と完全一致すること。 |
| `contains` | `value` | 出力に `value` が部分文字列として含まれること。 |
| `jq` | `expr` | `expr` を出力に対して評価し、真（jq の真偽値ルール）であること。出力テキストがそのまま有効な JSON であればその値に対して、そうでなければ JSON 文字列としてラップした値に対して評価されるので、`contains("...")` のような文字列アサーションと `.title \| length > 0` のような構造化アサーションのどちらも書けます。 |
| `llm_judge` | `criteria`、任意で `model`/`threshold` | LLM による採点（[出力品質の評価](./eval.md) を参照）。 |
| `tool_called` | `name`、任意で `min`/`max`/`args_jq` | ツールが呼ばれたこと（trajectory アサーション、後述）。 |
| `usage` | 任意で `max_prompt_tokens`/`max_completion_tokens`/`max_total_tokens`/`max_cost_usd` | トークン使用量・コストの上限（trajectory アサーション、後述）。 |
| `step_output` | `id`、`assert` | 名前付きステップ自身の出力に、ネストした `assert:` を適用（trajectory アサーション、後述）。 |

- `assert:` の各項目は上から順にすべて評価されます。
...
`assert:` で検査できます。`lait test`(1テストファイルにつき専用の `RunContext`)・`lait eval`
(各ケース/リピートごとに専用の `RunContext`、[出力品質の評価](./eval.md)を参照)のどちらでも、
これらのアサーションは常に**その1回の実行だけ**を見ます。
...
- `lait eval` でも同じ語彙が使えます。`--repeat`・複数ケースは並行実行されますが、各 (ケース, リピート)
  は専用の実行コンテキストを持つため、他のケース/リピートのイベントやトークン使用量が混ざることは
  ありません(詳しくは[出力品質の評価](./eval.md)を参照)。
