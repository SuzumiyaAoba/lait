...
| `lait -` と書いた場合 | 標準入力をプロンプトとして明示的に読み込みます。 |

標準入力の読み込み中も Ctrl-C で実行を中断できます。パイプの入力が終了するまで待っている場合も、
後続のモデル呼び出しへ進まずに終了します。
...
推論モデルに推論の実行レベルを指定する場合は、`--reasoning-effort` に次のいずれかを渡します。

| 値 | 説明 |
| --- | --- |
| `none` | 推論を行わない指定です（サーバーとモデルが対応している場合）。 |
| `minimal` / `low` / `medium` / `high` / `xhigh` | 推論の実行レベル（低 → 高の順）。 |

未指定時は `reasoning_effort` フィールドを送信せず、サーバー側の既定値を使用します。
...
`command` の実行、設定・workflow・agent・schema・eval/test 定義・checkpoint の読み込み、
標準入力の待機などをキャンセルしてから終了します（`--stream` で出力の途中だった場合、
