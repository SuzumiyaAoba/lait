...
## まず `lait doctor` を実行する

設定ファイルの構文、`${VAR}` で参照している環境変数、`default.model` の解決、各プロバイダーへの接続と認証、設定済みモデル ID がサーバー上に存在するか、`mcp_servers:` の起動・初期化、`agents:`/`skills:` が参照するファイルの存在を一括で診断します。原因を一つずつ手探りする前に、まずこれを実行してください。

```sh
lait doctor
```

各項目を `OK` / `WARN` / `NG` と対処ヒント付きで表示し、`NG` が一つでもあれば終了コードは非ゼロになります。`--json` を付けると機械可読な出力になり、CI での自動チェックにも使えます。
...
- ステップの `timeout:` はそのステップの1回の試行の制限です。ワークフロー全体の制限はトップレベルの `timeout:` で設定します（[ワークフロー](./workflow.md#エラー処理retry--timeout--on_error)）。
