...
| `--format` の値 | 出力 |
| --- | --- |
| `json` | `file`/`line`/`severity`/`message` を持つ構造化レコードの配列を標準出力に出力します。`line` は、YAML 自体のパースエラーであれば実際の行番号、それ以外の指摘（未使用ノードや不明な参照など）ではメッセージ中の最初のクォート識別子をファイル内で検索したベストエフォートの行番号です。特定できない場合は `null` になります（`lait.config.yml` 由来の指摘は常に `null` です）。 |
