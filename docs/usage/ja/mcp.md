...
- `deny` が先に評価されます。マッチすれば `allow` の内容に関わらず拒否されます。
- `allow` が空（既定）ならすべてのツールが許可対象です。非空の場合、マッチしないツールは拒否されます。
...
stderr に表示し、stdin で `y`（今回だけ許可）/ `n`（今回だけ拒否）/ `a`（このツール名を以後
このプロセス内では常に許可）を確認します。1 ラウンド内の複数のツール呼び出しは、実行前に
