...
| フィールド | 説明 |
| --- | --- |
| `command`（必須） | 実行するコマンドの argv。`command[0]` がプログラム、`command[1..]` が引数です。**シェルは介さず直接 exec されます**（`;`/`|`/バッククォートなどでコマンドを注入することはできません）。空リストは設定エラーです。 |
| `description`（任意） | モデルに見せるツールの説明。省略もできますが、モデルが正しく使うためには書いておくことを推奨します。 |
| `parameters`（任意） | モデルの呼び出し引数を表す JSON Schema。OpenAI のツール定義にそのまま渡されます。省略時は引数なしのツール（`{"type":"object","properties":{}}`）として扱われます。JSON オブジェクトである必要があります。実行前に `required`/`type`/`enum` と、`properties`/`items` の入れ子を検証します。これはよく使う制約を検証するもので、完全な JSON Schema 実装ではありません。 |
| `timeout`（任意、秒） | コマンドの実行時間の上限。省略時は 30 秒です。超過すると呼び出しはタイムアウトエラーとして扱われます。 |
| `env`（任意） | コマンドの子プロセスに渡す環境変数の**許可リスト**。1件以上指定すると、子プロセスはここに列挙した変数**だけ**を見ます — lait 自身が親プロセスから継承している環境変数（API キーなどの秘密情報を含みうる）は一切引き継がれません。省略時（既定）は、従来どおり親プロセスの環境変数をそのまま継承します。各値は `mcp_servers[]` の `env`/`cwd` と同じく `${VAR_NAME}` で環境変数を参照できます。 |
| `cwd`（任意） | コマンドの作業ディレクトリを固定します。省略時（既定）は lait 自身のカレントディレクトリを継承します。`${VAR_NAME}` 展開に対応します。 |

```yaml
tools:
  probe:
    command: ["sh", "-c", "echo $SAFE_TOKEN"]
    env:
      PATH: "${PATH}"          # command[0] を名前解決するために必要（下記の注意を参照）
      SAFE_TOKEN: "${SAFE_TOKEN}"
    cwd: "/tmp/lait-sandbox"
```

**注意**: `env` を1件でも指定すると環境変数は完全な許可リストになるため、`command[0]` が絶対パス
でない場合は `PATH` も明示的に含めてください。含めないと、コマンド自体が見つからずエラーになります
（`PATH` 自体は機密情報ではないので、多くの場合そのまま `${PATH}` として渡して構いません）。

標準出力と標準エラー出力はそれぞれ 16 MiB までを保持します。どちらかが上限を超えるとコマンドの
プロセスツリーを終了して回収し、ツールの結果にエラーを返します。これにより大量出力で tool loop が
メモリを使い続けることを防ぎます。

同じ `env`/`cwd` による封じ込めは、ワークフローの `run:` ステップ
（[ワークフロー](./workflow.md#run--コマンドを実行する)）でも指定できます。
...
引数は JSON オブジェクトで指定します。空文字列または `null` は空のオブジェクト（`{}`）として扱い、
配列・文字列・数値・真偽値などはコマンドを実行せずエラーにします。JSON Schema の実行時検証に失敗
した場合もコマンドは実行せず、その理由をツール結果としてモデルに返します。
...
同じ応答に複数の tool call がある場合は最大 8 件を並行して実行し、モデルが指定した順番で結果を
返します。
...
ユーザーのキャンセルは中断エラーとして上位へ伝播し、ツールの通常結果文字列には変換されません。
