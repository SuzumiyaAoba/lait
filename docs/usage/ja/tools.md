...
- `parameters`（任意）: モデルの呼び出し引数を表す JSON Schema。OpenAI のツール定義にそのまま渡され
  ...
  オブジェクトである必要があります。実行前に `required`/`type`/`enum` と、`properties`/`items` の
  入れ子を検証します。これはよく使う制約を検証するもので、完全な JSON Schema 実装ではありません。
- `timeout`（任意、秒）: コマンドの実行時間の上限。省略時は 30 秒です。超過すると呼び出しはタイム
  アウトエラーとして扱われます。
- `env`（任意）: コマンドの子プロセスに渡す環境変数の**許可リスト**。1件以上指定すると、子プロセスは
  ここに列挙した変数**だけ**を見ます — lait 自身が親プロセスから継承している環境変数（API キーなど
  の秘密情報を含みうる）は一切引き継がれません。省略時（既定）は、従来どおり親プロセスの環境変数を
  そのまま継承します。各値は `mcp_servers[]` の `env`/`cwd` と同じく `${VAR_NAME}` で環境変数を
  参照できます。
- `cwd`（任意）: コマンドの作業ディレクトリを固定します。省略時（既定）は lait 自身のカレント
  ディレクトリを継承します。`${VAR_NAME}` 展開に対応します。
