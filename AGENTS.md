...
When splitting a module `<name>.rs` into a `<name>/` directory, decide the shape mechanically: if, after moving code out, the parent file still holds anything besides `mod` declarations and `pub(crate) use` re-exports (a type definition, a function, an `impl` block, a constant), keep `<name>.rs` as a facade alongside `<name>/` (as `src/config.rs`+`src/config/` does); if nothing but `mod`/`use` remains, collapse to `<name>/mod.rs` (as `src/async_io/mod.rs` and `src/deps/mod.rs` do — `mod` declarations plus `use`/`pub(crate) use` re-exports and nothing else). Don't reclassify existing modules to fit this rule. Note that `jsonl/mod.rs` and `workflow/mod.rs` hold functions/structs, not only `mod`/`use` — the shape this rule assigns to the facade form — precisely because they predate it. `workflow/exec.rs`, `workflow/model.rs`, `workflow/parse.rs`, `workflow/inputs.rs`, and `workflow/tests.rs` are single files, not split modules.
...
- Each `tests/*.rs` file and `tests/workflow/` stays a separate integration test binary rather than being grouped into fewer themed ones. Measured (with 40 test targets at the time) after a `cargo clean && cargo build --tests --locked --timings` under `[profile.dev.package."*"] opt-level = 2`: the `lait`-crate test targets' wall-clock tail (their earliest start to their latest finish, since they compile in parallel once the shared `lait` rlib is ready) was ~15s against a ~197s total build, and their summed per-unit durations were ~8% of the summed duration of every unit in the build — both well under the 40% gate this decision was conditioned on. Caveats for a future re-measurement: `--timings` reports per-unit rustc totals, not an isolated link phase, so this is an estimate, not a measured link-time share; and the ~8% denominator was inflated by `opt-level = 2` raising dependency-compile cost, so a re-measure under a different profile will not reproduce this number and needs its own run.
- `lait lint <DIR>`'s recursive directory walk skips dot-prefixed *files* as well as dot-prefixed directories, because it shares `file_walk::DirWalker` with `lait test` and the two must agree on what a directory contains (see `src/lint/targets.rs`'s dot-file test and `docs/usage/ja/lint.md`'s directory-walk paragraph). An explicitly named file argument is never skipped.
...
Keep API keys in environment variables or an untracked `.env`; the root `.gitignore` already ignores `.env`/`.env.*`, but double-check before committing config files that embed secrets directly. `lait.config.yml`'s top-level `base_url`/`api_key`, `models[].provider.*`, `jev.base_url`/`jev.api_key`, `mcp_servers[]` (`command`/`args`/`env`/`cwd`/`url`/`headers`), `tools.<name>.env`/`.cwd`, and a workflow `run:` step's `env`/`cwd` support `${VAR_NAME}` expansion — other fields (prompt templates, `default.system`, workflow `prompt:`/`system:`) do not. `--no-env`/`--no-config` can disable local loading. Treat `mcp_servers` entries as trusted code: they may launch child processes or connect to remote URLs, and credentials must not be hard-coded.
